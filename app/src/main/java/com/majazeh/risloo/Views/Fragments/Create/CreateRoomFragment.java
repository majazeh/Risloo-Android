    ...
    private void listener() {
        ...
        ClickManager.onDelayedClickListener(() -> {
            if (binding.psychologyErrorLayout.getRoot().getVisibility() == View.VISIBLE)
                ((MainActivity) requireActivity()).validatoon.check(binding.psychologyErrorLayout.getRoot(), binding.psychologyErrorLayout.errorTextView);
                                    ...
                                    for (int i = 0; i < errorsObject.getJSONArray(key).length(); i++) {
                                        ...
                                        switch (key) {
                                            case "psychologist_id":
                                                ((MainActivity) requireActivity()).validatoon.error(binding.psychologyErrorLayout.getRoot(), binding.psychologyErrorLayout.errorTextView, validation);
                                                break;
                                        }
